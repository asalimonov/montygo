...
func (p *clsPoint) SumAsync() *monty.Future {
	return monty.Async(func() (any, error) { return p.Sum(), nil })
}
...
func TestWasmClassInstance(t *testing.T) {
	...
	t.Run("a ClassInstance round-trips over the wasm transport", func(t *testing.T) {
		...
		opts := &monty.FeedOptions{Inputs: map[string]any{"p": clsInstance(t, point, monty.ClassInstanceOptions{EagerAttrs: monty.All, AllowedMethods: monty.All})}}
	...
	t.Run("async host methods use one suspension per call over wasm", func(t *testing.T) {
		ctx := testCtx(t)
		s := newSession(t, b, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxSuspensions: 2}})
		point := clsInstance(t, &clsPoint{X: 1, Y: 2}, monty.ClassInstanceOptions{AllowedMethods: monty.All})
		v, err := s.FeedRun(ctx, "a = await p.sum_async()\nb = await p.sum_async()\na + b", &monty.FeedOptions{Inputs: map[string]any{"p": point}})
		require.NoError(t, err)
	...
	t.Run("a ClassType round-trips over the wasm transport", func(t *testing.T) {
		ctx := testCtx(t)
		s := newSession(t, b, monty.CheckoutOptions{})
		wrapper := clsType[clsPoint](t, monty.ClassTypeOptions{
			Init:               true,
		...
		opts := &monty.FeedOptions{Inputs: map[string]any{"Point": wrapper}}
	...
	t.Run("a MontyClassProxy round-trips over the wasm transport", func(t *testing.T) {
		...
		require.True(t, monty.Equal(clsDict("x", int64(1)), proxy.Attributes), monty.Repr(proxy.Attributes))
		v, err = s.FeedRun(ctx, "back is foo and back.x == 1", &monty.FeedOptions{Inputs: map[string]any{"back": proxy}})
		require.NoError(t, err)
		require.Equal(t, true, v)
	})
}
