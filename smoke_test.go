...
func TestSmoke(t *testing.T) {
	eachBackend(t, func(t *testing.T, b backend) {
		...
		t.Run("inputs and containers", func(t *testing.T) {
			...
			tup, _ := d.Get("t")
			require.Equal(t, sandbox.Tuple{int64(1), int64(2)}, tup)
			require.Equal(t, "{'a': 11, 't': (1, 2), 's': {3}}", sandbox.Repr(v))
		})
		t.Run("external function", func(t *testing.T) {
			v := mustRun(t, b, "add(2, 3) + scale(4, factor=10)", runOptions{FeedOptions: montygo.FeedOptions{ExternalLookup: map[string]any{
				...
				"scale": func(x int, kw host.Kwargs) int { return x * int(kw["factor"].(int64)) },
			}}})
		...
		t.Run("async futures", func(t *testing.T) {
			v := mustRun(t, b, "import asyncio\nawait asyncio.gather(fetch(1), fetch(2))", runOptions{FeedOptions: montygo.FeedOptions{ExternalLookup: map[string]any{
				"fetch": func(n int) *host.Future {
					return host.Async(func() (any, error) { time.Sleep(10 * time.Millisecond); return n * 10, nil })
				},
			}}})
		...
		t.Run("runtime error", func(t *testing.T) {
			...
			require.Contains(t, rt.Display(monterr.DisplayTraceback), "Traceback (most recent call last):")
		})
		t.Run("host error type", func(t *testing.T) {
			v := mustRun(t, b, "try:\n    boom()\nexcept ValueError as e:\n    r = str(e)\nr", runOptions{FeedOptions: montygo.FeedOptions{ExternalLookup: map[string]any{
				"boom": func() error { return monterr.Raise("ValueError", "bad") },
			}}})
			require.Equal(t, "bad", v)
		})
		t.Run("syntax error", func(t *testing.T) {
			_, err := run(t, b, "def", runOptions{})
			var se *monterr.SyntaxError
			require.ErrorAs(t, err, &se)
		})
		t.Run("type check", func(t *testing.T) {
			_, err := run(t, b, "x: int = 'a'", runOptions{Runtime: mustRuntime(montygo.RuntimeOptions{TypeCheck: true}), CheckoutOptions: montygo.CheckoutOptions{}})
			var te *monterr.TypingError
			require.ErrorAs(t, err, &te)
			require.NotEmpty(t, te.Diagnostics)
		})
		t.Run("print collector", func(t *testing.T) {
			c, err := sandbox.NewCollectStreams(sandbox.DefaultMaxPrintCollectBytes)
			require.NoError(t, err)
			mustRun(t, b, "import sys\nprint('hello')\nprint('err', file=sys.stderr)", runOptions{FeedOptions: montygo.FeedOptions{Print: c}})
			out := c.Output()
			joined := map[sandbox.Stream]string{}
			for _, e := range out {
				joined[e.Stream] += e.Text
			}
			require.Equal(t, "hello\n", joined[sandbox.Stdout])
			require.Equal(t, "err\n", joined[sandbox.Stderr])
		})
		t.Run("print callback error", func(t *testing.T) {
			sentinel := errors.New("stop")
			_, err := run(t, b, "print('x')", runOptions{FeedOptions: montygo.FeedOptions{Print: sandbox.PrintFunc(func(sandbox.Stream, string) error { return sentinel })}})
			require.ErrorIs(t, err, sentinel)
		})
		t.Run("class instance", func(t *testing.T) {
			p := &smokePerson{Name: "Samuel", Age: 4}
			ci, err := host.NewClassInstance(p, host.ClassInstanceOptions{EagerAttrs: host.All(), AllowedMethods: host.Names("greeting")})
			require.NoError(t, err)
		...
		t.Run("class type init", func(t *testing.T) {
			ct, err := host.NewClassType[smokePerson](host.ClassTypeOptions{Init: true, InstanceEagerAttrs: host.All(), InstanceAllowedMethods: host.All()})
			require.NoError(t, err)
		...
		t.Run("name lookup value and os", func(t *testing.T) {
			v := mustRun(t, b, "import os\ngreeting + os.getenv('HOME')", runOptions{Runtime: mustRuntime(montygo.RuntimeOptions{OS: func(_ context.Context, name string, args []any, _ host.Kwargs) (any, error) {
				if name == "os.getenv" && args[0] == "HOME" {
					return "/home/user", nil
				}
				return host.NotHandled, nil
			}}), FeedOptions: montygo.FeedOptions{ExternalLookup: map[string]any{"greeting": "hello "}}})
