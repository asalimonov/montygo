...
func run(ctx context.Context, out io.Writer) error {
	...
	err = withSession(ctx, pool, func(session *monty.Session) error {
		wrapper, err := monty.NewClassType[Person](monty.ClassTypeOptions{Init: true, InstanceEagerAttrs: monty.All, InstanceAllowedMethods: monty.All})
		if err != nil {
			return err
		}
		result, err := session.FeedRun(ctx, "p = Person(\"Samuel\", 4)\np.greeting()", &monty.FeedOptions{Inputs: map[string]any{"Person": wrapper}})
		if err != nil {
			return err
		}
		if result != "hi Samuel" {
	...
	return withSession(ctx, pool, func(session *monty.Session) error {
		wrapper, err := monty.NewClassType[Person](monty.ClassTypeOptions{})
		if err != nil {
			return err
		}
		_, err = session.FeedRun(ctx, "Person(\"Samuel\", 4)", &monty.FeedOptions{Inputs: map[string]any{"Person": wrapper}})
