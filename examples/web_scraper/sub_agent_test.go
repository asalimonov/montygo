...
func pyDict(kv ...any) *sandbox.Dict {
	d := sandbox.NewDict()
...
func TestRecordModelInfoValidInputSkipsSubAgent(t *testing.T) {
	...
	got, err := records.recordModelInfo(t.Context(), nil, host.Kwargs{"model_information": pyDict("unique_id", "m", "name", "M", "input_mtok", 1.0, "output_mtok", 2.0)})
	require.NoError(t, err)
