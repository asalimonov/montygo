...
Each directory is a `package main` whose sandbox code is byte-identical to upstream. `repl`, `oneshot` and `service` have no upstream example: `repl` ports the REPL of the `monty` CLI, and the other two show the montygo-only lifecycle API.
...
| `oneshot` | — | `Pool.Run`: checkout, feed and close in one call |
| `service` | — | A long-running loop in a `Slot` with a `Host` (`sleep`, an object exposed through `Expose`), stopped on SIGINT by `Run.Stop` with the default policy, then `Slot.Close` and `Pool.Shutdown` |
...
- **repl** reads Python from stdin: `go run ./repl`, or `go run ./repl -c 'x = 1'` / `go run ./repl setup.py` to run code before the first prompt. It takes the `monty` CLI flags `-m host::/virtual[::ro|rw|overlay[::write_limit]]`, `--cwd`, `--max-duration`, `--max-memory`, `--gc-interval`, `--max-recursion-depth` and `--max-suspensions`. `-ws URL` runs the sessions on a remote `monty-server` instead of local workers, for example `docker run --rm -p 127.0.0.1:8000:8000 monty-server:latest` after `make docker-build`, then `go run ./repl -ws ws://127.0.0.1:8000/`. For `wss://`, `-ws-ca FILE` trusts a PEM CA bundle and `-ws-insecure-skip-verify` disables certificate checks; both require `-ws`. Upstream decides whether input is complete by parsing it in-process; the Go REPL feeds the pending text and reads the syntax error instead, so traceback file names (`<python-input-N>`) skip numbers used by incomplete input. Overlay mount writes last one snippet. Ctrl-C during a run calls `Run.Stop` with the default policy: a snippet waiting on the host is aborted and the session is kept; a snippet that never yields is killed after 3 s and a new session starts.
- **oneshot** prints `sum(range(10))` through `pool.Run`: `go run ./oneshot`.
- **service** runs an endless loop that prints a tick every 200 ms until SIGINT, then prints the stop kind (`aborted`): `go run ./service`, Ctrl-C to stop. `-run-for 2s` stops after a duration instead of a signal, which is how its test runs it. `-ws URL` runs the session on a remote `monty-server`.
