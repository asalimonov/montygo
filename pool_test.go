...
func plNativeOnly(t *testing.T, b monty.Backend, reason string) {
	t.Helper()
	if b != monty.BackendNative {
		t.Skipf("%s backend: %s", b, reason)
	}
}

func plPID(t *testing.T, s *monty.Session) int {
	t.Helper()
	pid, ok := s.WorkerPID()
...
func plCheckout(t *testing.T, p *monty.Pool, opts monty.CheckoutOptions) *monty.Session {
	t.Helper()
...
func plFeed(t *testing.T, s *monty.Session, code string, opts *monty.FeedOptions) any {
	t.Helper()
...
func plClose(t *testing.T, s *monty.Session) {
	t.Helper()
...
func TestPool(t *testing.T) {
	eachBackend(t, func(t *testing.T, b monty.Backend) {
		t.Run("checkout after close rejects", func(t *testing.T) {
			...
			_, err := p.Checkout(testCtx(t), monty.CheckoutOptions{})
		...
		t.Run("workers are reused across checkouts", func(t *testing.T) {
			...
			first := plCheckout(t, p, monty.CheckoutOptions{})
			pid := plPID(t, first)
			plClose(t, first)
			second := plCheckout(t, p, monty.CheckoutOptions{})
		...
		t.Run("maxCheckoutsPerWorker recycles the worker", func(t *testing.T) {
			plNativeOnly(t, b, "worker identity is observed through the worker pid")
			p := newPool(t, b, monty.Options{MaxCheckoutsPerWorker: 1})
			first := plCheckout(t, p, monty.CheckoutOptions{})
			pid := plPID(t, first)
			plClose(t, first)
			second := plCheckout(t, p, monty.CheckoutOptions{})
		...
		t.Run("maxMemory leaves normal work alone", func(t *testing.T) {
			p := newPool(t, b, monty.Options{})
			s := plCheckout(t, p, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxMemory: 1024 * 1024}})
		...
		t.Run("concurrent sessions run in distinct workers", func(t *testing.T) {
			p := newPool(t, b, monty.Options{MaxProcesses: 2})
			a := plCheckout(t, p, monty.CheckoutOptions{})
			c := plCheckout(t, p, monty.CheckoutOptions{})
			if b == monty.BackendNative {
		...
		t.Run("exhausted pool times out the checkout", func(t *testing.T) {
			p := newPool(t, b, monty.Options{MaxProcesses: 1, CheckoutTimeout: 200 * time.Millisecond})
			held := plCheckout(t, p, monty.CheckoutOptions{})
			_, err := p.Checkout(testCtx(t), monty.CheckoutOptions{})
			require.ErrorIs(t, err, monty.ErrCheckoutTimeout)
		...
		t.Run("session is unusable after a crash but the pool recovers", func(t *testing.T) {
			...
			plClose(t, s)
			fresh := plCheckout(t, p, monty.CheckoutOptions{})
		...
		t.Run("requestTimeout kills a wedged worker", func(t *testing.T) {
			p := newPool(t, b, monty.Options{RequestTimeout: 500 * time.Millisecond})
			s := plCheckout(t, p, monty.CheckoutOptions{})
			_, err := s.FeedRun(testCtx(t), "while True:\n    pass", nil)
			crashed := plRequireCrashed(t, err)
		...
		t.Run("suspension time does not consume the duration budget", func(t *testing.T) {
			p := newPool(t, b, monty.Options{})
			s := plCheckout(t, p, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxDuration: 300 * time.Millisecond}})
			v := plFeed(t, s, "await fetch_data('u') + '!'", &monty.FeedOptions{ExternalLookup: map[string]any{
				"fetch_data": func(string) *monty.Future {
