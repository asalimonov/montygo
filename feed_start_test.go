...
func fstResumeAuto(ctx context.Context, t *testing.T, snap monty.Snapshot) (monty.Snapshot, error) {
	t.Helper()
	switch s := snap.(type) {
...
func fstAs[T monty.Snapshot](t *testing.T) func(monty.Snapshot, error) T {
	return func(snap monty.Snapshot, err error) T {
		t.Helper()
...
func fstComplete(t *testing.T) func(monty.Snapshot, error) *monty.Complete {
	return fstAs[*monty.Complete](t)
}

func fstFunction(t *testing.T) func(monty.Snapshot, error) *monty.FunctionSnapshot {
	return fstAs[*monty.FunctionSnapshot](t)
}

func fstNameLookup(t *testing.T) func(monty.Snapshot, error) *monty.NameLookupSnapshot {
	return fstAs[*monty.NameLookupSnapshot](t)
}

func fstRuntimeError(t *testing.T, err error) *monty.RuntimeError {
	t.Helper()
	require.Error(t, err)
...
func fstDrive(ctx context.Context, t *testing.T, snap monty.Snapshot, err error) (*monty.Complete, int) {
	t.Helper()
	steps := 0
	for {
		require.NoError(t, err)
		if done, ok := snap.(*monty.Complete); ok {
			return done, steps
		}
...
func TestFeedStart(t *testing.T) {
	eachBackend(t, func(t *testing.T, b monty.Backend) {
		...
		t.Run("a snapshot resumes at most once", func(t *testing.T) {
			...
			require.ErrorIs(t, err, monty.ErrSnapshotResumed)
		...
		t.Run("os handler is used by resumeAuto, not auto-dispatched", func(t *testing.T) {
			...
			session := newSession(t, b, monty.CheckoutOptions{})
			var names []string
			snap := fstFunction(t)(session.FeedStart(ctx, "from pathlib import Path\nPath('/data/x').read_text()", &monty.FeedOptions{
				OS: func(_ context.Context, name string, _ []any, _ monty.Kwargs) (any, error) {
		...
		t.Run("the sandbox future mechanism is caller-driven", func(t *testing.T) {
			...
			require.NoError(t, err)
			futures, ok := snap.(*monty.FutureSnapshot)
			require.Truef(t, ok, "expected *monty.FutureSnapshot, got %T", snap)
			require.Equal(t, []uint32{call.CallID}, futures.PendingCallIDs)
			done := fstComplete(t)(futures.Resume(ctx, []monty.FutureResolution{{CallID: call.CallID, Value: 99}}))
		...
		t.Run("loadSession and loadSnapshot reject the wrong dump kind", func(t *testing.T) {
			ctx := testCtx(t)
			idleSession := newSession(t, b, monty.CheckoutOptions{})
			idle, err := idleSession.Dump(ctx)
			require.NoError(t, err)
			require.NoError(t, idleSession.Close(ctx))

			suspendedSession := newSession(t, b, monty.CheckoutOptions{})
...

			{
				...
				require.ErrorIs(t, err, monty.ErrDumpIsIdle)
			...
			}
			{
				...
				require.ErrorIs(t, err, monty.ErrDumpIsSuspended)
		...
		t.Run("load after a feed is rejected", func(t *testing.T) {
			...
			require.ErrorIs(t, err, monty.ErrNotFresh)
		...
		t.Run("resumeAuto answers a function call from externalLookup", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstFunction(t)(session.FeedStart(ctx, "add(2, 3) * 10", &monty.FeedOptions{
		...
		t.Run("resumeAuto drives a snippet to completion", func(t *testing.T) {
			...
			snap, err := session.FeedStart(ctx, code, &monty.FeedOptions{ExternalLookup: lookup})
		...
		t.Run("resumeAuto resolves a name lookup to a value", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstNameLookup(t)(session.FeedStart(ctx, "missing + 1", &monty.FeedOptions{
		...
		t.Run("resumeAuto with a missing name raises NameError", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstNameLookup(t)(session.FeedStart(ctx, "missing + 1", &monty.FeedOptions{ExternalLookup: map[string]any{}}))
		...
		t.Run("resumeAuto with a function absent from the lookup raises NameError", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstFunction(t)(session.FeedStart(ctx, "add(2, 3)", &monty.FeedOptions{ExternalLookup: map[string]any{}}))
			_, err := snap.ResumeAuto(ctx)
			rerr := fstRuntimeError(t, err)
		...
		t.Run("resumeAuto settles an immediately awaited promise without a FutureSnapshot", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstFunction(t)(session.FeedStart(ctx, fstAsyncGo, &monty.FeedOptions{
				ExternalLookup: map[string]any{"go": func() *monty.Future {
					return monty.Async(func() (any, error) { return 99, nil })
				}},
			}))
		...
		t.Run("resumeAuto drives multiple pending promises via gather", func(t *testing.T) {
			...
			snap, err := session.FeedStart(ctx, code, &monty.FeedOptions{
				ExternalLookup: map[string]any{"go": func(n int64) *monty.Future {
					return monty.Async(func() (any, error) { return n * 10, nil })
				}},
			})
			done, _ := fstDrive(ctx, t, snap, err)
		...
		t.Run("resumeAuto resumes at most once", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, monty.CheckoutOptions{})
			snap := fstFunction(t)(session.FeedStart(ctx, "add(1, 2)", &monty.FeedOptions{
				ExternalLookup: map[string]any{"add": func(a, b int64) int64 { return a + b }},
			}))
			_, err := snap.ResumeAuto(ctx)
			require.NoError(t, err)
		...
		t.Run("loadSnapshot captures externalLookup for resumeAuto", func(t *testing.T) {
			...
			session := newSession(t, b, monty.CheckoutOptions{})
			restored := fstFunction(t)(session.LoadSnapshot(ctx, blob, &monty.LoadSnapshotOptions{
