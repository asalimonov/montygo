...
func Func(fn any) (Function, error) {
	...
	if rv.Kind() != reflect.Func || rv.IsNil() {
		return nil, &monterr.ValueError{Message: fmt.Sprintf("Func expects a function, got %T", fn)}
	}
	rt := rv.Type()
	...
	if rt.NumOut() > 2 || (rt.NumOut() == 2 && rt.Out(1) != errorType) {
		return nil, &monterr.ValueError{Message: fmt.Sprintf("%s: results must be (T), (error) or (T, error)", rf.name)}
	}
	return rf, nil
}
...
func typeErr(format string, args ...any) error {
	return &monterr.RaisedError{ExcType: "TypeError", Message: fmt.Sprintf(format, args...)}
}
