...
func depthError() error { return &monterr.ConversionError{Message: "Max input depth exceeded"} }
...
func prepareInner(v any, store *InstanceStore, depth int) (any, error) {
	...
	case value.Time:
		if x.TimezoneName != nil && x.OffsetSeconds == nil {
			return nil, &monterr.ConversionError{Message: "MontyTime timezoneName requires offsetSeconds"}
		}
		return x, nil
	case value.DateTime:
		if x.TimezoneName != nil && x.OffsetSeconds == nil {
			return nil, &monterr.ConversionError{Message: "MontyDateTime timezoneName requires offsetSeconds"}
		}
		return x, nil
	case []byte:
		return x, nil
	...
	case value.Instance, *value.Instance:
		return nil, &monterr.ConversionError{Message: "raw ClassInstance markers are not accepted — wrap the object in ClassInstance(...)"}
	case value.Type:
		if x.Origin != value.OriginBuiltin {
			return nil, &monterr.ConversionError{Message: "raw Type markers are not accepted — pass the class through ClassType(...)"}
		}
		return x, nil
	case value.Cycle:
		return nil, &monterr.ConversionError{Message: "Cannot convert cycle marker to Monty value"}
	...
	return nil, &monterr.ConversionError{Message: fmt.Sprintf("Cannot convert Go %T to Monty value", v)}
}
...
func AsNamedTuple(v any) (sandbox.NamedTuple, error) {
	rv := reflect.ValueOf(v)
	for rv.Kind() == reflect.Pointer {
		if rv.IsNil() {
			return sandbox.NamedTuple{}, &monterr.ConversionError{Message: "AsNamedTuple expects a struct, got nil"}
		}
		rv = rv.Elem()
	}
	if rv.Kind() != reflect.Struct {
		return sandbox.NamedTuple{}, &monterr.ConversionError{Message: fmt.Sprintf("AsNamedTuple expects a struct, got %T", v)}
	}
	members := memberIndex(rv.Type())
	out := sandbox.NamedTuple{TypeName: rv.Type().Name(), FieldNames: append([]string(nil), members.fieldNames...)}
	if out.TypeName == "" {
...
func NewNamedTuple(typeName string, pairs ...sandbox.Pair) (sandbox.NamedTuple, error) {
	if typeName == "" {
		return sandbox.NamedTuple{}, &monterr.ValueError{Message: "NewNamedTuple: type name must not be empty"}
	}
	out := sandbox.NamedTuple{TypeName: typeName, FieldNames: make([]string, 0, len(pairs)), Values: make([]any, 0, len(pairs))}
	seen := map[string]struct{}{}
	for _, p := range pairs {
		name, ok := p.Key.(string)
		if !ok || name == "" {
			return sandbox.NamedTuple{}, &monterr.ValueError{Message: fmt.Sprintf("NewNamedTuple: field name must be a non-empty string, got %v", p.Key)}
		}
		if _, dup := seen[name]; dup {
			return sandbox.NamedTuple{}, &monterr.ValueError{Message: fmt.Sprintf("NewNamedTuple: duplicate field %q", name)}
		}
		seen[name] = struct{}{}
...
func wrapHint(t reflect.Type) error {
	...
	return &monterr.ConversionError{Message: fmt.Sprintf("Cannot convert %s instance to a Monty value — wrap it in ClassInstance(...)", name)}
}
