...
func collectLimit(maxBytes int64) (int64, error) {
	if maxBytes < UnlimitedPrintCollect {
		return 0, &monterr.OptionError{Message: "maxBytes must be a finite non-negative number or null"}
	}
	return maxBytes, nil
}

func collectOverflow(used, limit int64) error {
	return &monterr.RuntimeError{TypeName: "MemoryError", Message: fmt.Sprintf("memory limit exceeded: %d bytes > %d bytes", used, limit)}
}
