...
func NewMountDir(opts MountDirOptions) (*MountDir, error) {
	...
	if err != nil {
		return nil, &monterr.ValueError{Message: fmt.Sprintf("invalid mount mode: '%s'. Expected 'read-only', 'read-write' or 'overlay'", mode)}
	}
...
func BuildMounts(mounts []*MountDir) (pool.MountTable, string, error) {
	...
	for _, m := range mounts {
		...
		if m.root.Closed() {
			return nil, "", &monterr.OptionError{Message: "mount is closed: create a new MountDir"}
		}
