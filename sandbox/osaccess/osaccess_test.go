...
func newCustomFile(path, content string) *customFile {
	return &customFile{path: sandbox.Path(path), name: path[strings.LastIndex(path, "/")+1:], permissions: 0o644, content: content}
}

func (f *customFile) Path() sandbox.Path        { return f.path }
func (f *customFile) SetPath(p sandbox.Path)    { f.path = p }
...
func testInit(t *testing.T) {
	...
	montyTest(t, "test_empty_initialization", func(t *testing.T, b backend) {
		require.Equal(t, false, mustRun(t, b, `from pathlib import Path; Path("/any/path").exists()`, newFS(t).Handler()))
	})

	montyTest(t, "test_environ_parameter", func(t *testing.T, b backend) {
	...
	t.Run("test_time_methods_direct_api", func(t *testing.T) {
		...
		aware, err := fs.DatetimeNow(&sandbox.TimeZone{OffsetSeconds: 0})
		require.NoError(t, err)
...
func testExistence(t *testing.T) {
	montyTest(t, "test_path_exists_file", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_path_exists_directory", func(t *testing.T, b backend) {
...
func testReading(t *testing.T) {
	...
	montyTest(t, "test_read_text_bytes_content_decoded", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_read_bytes_bytes_content", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_read_bytes_string_content_encoded", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello"))
		require.Equal(t, []byte("hello"), mustRun(t, b, `from pathlib import Path; Path("/test/file.txt").read_bytes()`, fs.Handler()))
	})

	montyTest(t, "test_read_text_file_not_found", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_read_bytes_file_not_found", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_read_text_is_a_directory", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
	...
	montyTest(t, "test_read_bytes_is_a_directory", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
...
func testWriting(t *testing.T) {
	montyTest(t, "test_write_text_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_write_text_overwrite_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_write_bytes_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/existing.txt", "existing"))
	...
	montyTest(t, "test_write_text_parent_not_exists_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_write_text_to_directory_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
...
`
	...
	montyTest(t, "test_open_directory_raises_is_a_directory", func(t *testing.T, b backend) {
...
`
	...
	montyTest(t, "test_open_write_truncates_existing", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_open_write_creates_missing", func(t *testing.T, b backend) {
		fs := newFS(t)
	...
	montyTest(t, "test_open_write_then_write_data", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_open_append_preserves_existing", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/data/log.txt", "keep me"))
		mustRun(t, b, "\nf = open('/data/log.txt', 'a')\nf.write('!')\nf.close()\n", fs.Handler())
		require.Equal(t, "keep me!", must[string](t)(fs.PathReadText("/data/log.txt")))
	})

	montyTest(t, "test_open_append_creates_missing", func(t *testing.T, b backend) {
		fs := newFS(t)
		mustRun(t, b, "\nf = open('/fresh.txt', 'a')\nf.write('seed')\nf.close()\n", fs.Handler())
		require.Equal(t, "seed", must[string](t)(fs.PathReadText("/fresh.txt")))
	})

	montyTest(t, "test_open_append_text_non_ascii_returns_char_count", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_open_binary_write_returns_byte_count", func(t *testing.T, b backend) {
		fs := newFS(t)
		code := `
f = open('/out.bin', 'wb')
...
`
	...
	montyTest(t, "test_open_write_to_read_only_raises", func(t *testing.T, b backend) {
...
`
	...
	montyTest(t, "test_open_read_from_write_only_raises", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/data/file.txt", "x"))
		code := `
f = open('/data/file.txt', 'w')
try:
...
`
	...
	montyTest(t, "test_open_keyword_args", func(t *testing.T, b backend) {
	...
	// Python checks isinstance(handle, MontyFileHandle); Go returns *sandbox.FileHandle by type.
	t.Run("test_path_open_returns_monty_file_handle", func(t *testing.T) {
		fs := newFS(t, mem("/data/file.txt", "x"))
		handle := must[*sandbox.FileHandle](t)(fs.PathOpen("/data/file.txt", "r"))
	...
	t.Run("test_path_open_normalizes_mode", func(t *testing.T) {
		fs := newFS(t, mem("/data/file.txt", "x"))
		require.Equal(t, "r", must[*sandbox.FileHandle](t)(fs.PathOpen("/data/file.txt", "rt")).Mode)
	})
	...
	t.Run("test_path_open_w_truncates_via_direct_api", func(t *testing.T) {
		fs := newFS(t, mem("/data/file.txt", "previous"))
		must[*sandbox.FileHandle](t)(fs.PathOpen("/data/file.txt", "w"))
		require.Equal(t, "", must[string](t)(fs.PathReadText("/data/file.txt")))
	})
...
func testMkdirRmdir(t *testing.T) {
	montyTest(t, "test_mkdir_basic_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello"))
		mustRun(t, b, "\nfrom pathlib import Path\nPath('/test/newdir').mkdir()\n", fs.Handler())
		require.True(t, must[bool](t)(fs.PathIsDir("/test/newdir")))
	})

	montyTest(t, "test_mkdir_with_parents_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t)
	...
	montyTest(t, "test_mkdir_exist_ok_true_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
		mustRun(t, b, "\nfrom pathlib import Path\nPath('/test/subdir').mkdir(exist_ok=True)\n", fs.Handler())
		require.True(t, must[bool](t)(fs.PathIsDir("/test/subdir")))
	})

	montyTest(t, "test_mkdir_exist_ok_false_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
	...
	montyTest(t, "test_mkdir_parent_not_exists_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_rmdir_empty_directory_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
		require.NoError(t, fs.PathMkdir("/test/newdir", false, false))
	...
	montyTest(t, "test_rmdir_non_empty_directory_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
	...
	montyTest(t, "test_rmdir_not_found_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_rmdir_file_not_directory_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello"))
...
func testIterdirUnlinkStat(t *testing.T) {
	montyTest(t, "test_iterdir_list_contents", func(t *testing.T, b backend) {
	...
	t.Run("test_iterdir_empty_directory_direct", func(t *testing.T) {
		...
		require.Empty(t, must[[]sandbox.Path](t)(fs.PathIterdir("/test/empty")))
	})
	...
	montyTest(t, "test_iterdir_not_found", func(t *testing.T, b backend) {
		_, err := run(t, b, "from pathlib import Path; list(Path('/missing').iterdir())", newFS(t).Handler())
		requireRuntimeError(t, err, "FileNotFoundError: [Errno 2] No such file or directory: '/missing'")
	})

	montyTest(t, "test_unlink_file_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello"))
		mustRun(t, b, "\nfrom pathlib import Path\nPath('/test/file.txt').unlink()\n", fs.Handler())
		require.False(t, must[bool](t)(fs.PathExists("/test/file.txt")))
	})

	montyTest(t, "test_unlink_file_not_found_via_monty", func(t *testing.T, b backend) {
		_, err := run(t, b, "from pathlib import Path; Path('/missing.txt').unlink()", newFS(t).Handler())
		requireRuntimeError(t, err, "FileNotFoundError: [Errno 2] No such file or directory: '/missing.txt'")
	})

	montyTest(t, "test_unlink_is_directory_via_monty", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
	...
	montyTest(t, "test_stat_file", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello world"))
		result := mustRun(t, b, "\nfrom pathlib import Path\ns = Path('/test/file.txt').stat()\n(s.st_size, s.st_mode & 0o777)\n", fs.Handler())
		require.Equal(t, sandbox.Tuple{int64(11), int64(0o644)}, result)
	})

	montyTest(t, "test_stat_file_custom_permissions", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello", 0o755))
		result := mustRun(t, b, "\nfrom pathlib import Path\ns = Path('/test/file.txt').stat()\ns.st_mode & 0o777\n", fs.Handler())
		require.Equal(t, int64(0o755), result)
	})

	montyTest(t, "test_stat_directory", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/subdir/file.txt", "hello"))
		result := mustRun(t, b, "\nfrom pathlib import Path\ns = Path('/test/subdir').stat()\ns.st_mode\n", fs.Handler())
		require.Equal(t, int64(0o040755), result)
	})

	montyTest(t, "test_stat_file_not_found", func(t *testing.T, b backend) {
		_, err := run(t, b, "from pathlib import Path; Path('/missing').stat()", newFS(t).Handler())
		requireRuntimeError(t, err, "FileNotFoundError: [Errno 2] No such file or directory: '/missing'")
	})

	montyTest(t, "test_stat_bytes_content_size", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.bin", []byte{0, 1, 2, 3, 4}))
		require.Equal(t, int64(5), mustRun(t, b, "\nfrom pathlib import Path\nPath('/test/file.bin').stat().st_size\n", fs.Handler()))
	})

	montyTest(t, "test_stat_unicode_size", func(t *testing.T, b backend) {
...
func testRenameResolve(t *testing.T) {
	montyTest(t, "test_rename_file_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_rename_source_not_found_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_rename_target_parent_not_found_via_monty", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_path_resolve_absolute", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/test/file.txt", "hello"))
		require.Equal(t, "/test/file.txt", mustRun(t, b, "\nfrom pathlib import Path\nstr(Path('/test/file.txt').resolve())\n", fs.Handler()))
	})

	montyTest(t, "test_path_absolute_already_absolute", func(t *testing.T, b backend) {
		require.Equal(t, "/already/absolute", mustRun(t, b, "\nfrom pathlib import Path\nstr(Path('/already/absolute').absolute())\n", newFS(t).Handler()))
	})

	montyTest(t, "test_path_absolute_relative", func(t *testing.T, b backend) {
		require.Equal(t, "/relative/path", mustRun(t, b, "\nfrom pathlib import Path\nstr(Path('relative/path').absolute())\n", newFS(t).Handler()))
	})

	montyTest(t, "test_path_resolve_same_as_absolute", func(t *testing.T, b backend) {
...
func testEnviron(t *testing.T) {
	montyTest(t, "test_getenv_existing_key", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"MY_VAR": "my_value"})
		require.Equal(t, "my_value", mustRun(t, b, "import os; os.getenv('MY_VAR')", fs.Handler()))
	})

	montyTest(t, "test_getenv_missing_key", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"OTHER": "value"})
		require.Nil(t, mustRun(t, b, "import os; os.getenv('MISSING')", fs.Handler()))
	})

	montyTest(t, "test_getenv_missing_with_default", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_getenv_multiple_vars", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"VAR1": "value1", "VAR2": "value2", "VAR3": "value3"})
		result := mustRun(t, b, "\nimport os\n(os.getenv('VAR1'), os.getenv('VAR2'), os.getenv('VAR3'))\n", fs.Handler())
		require.Equal(t, sandbox.Tuple{"value1", "value2", "value3"}, result)
	})

	montyTest(t, "test_get_environ_returns_dict", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_get_environ_key_access", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"MY_VAR": "my_value"})
		require.Equal(t, "my_value", mustRun(t, b, "import os; os.environ['MY_VAR']", fs.Handler()))
	})

	montyTest(t, "test_get_environ_key_missing_raises", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_get_environ_get_method", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_get_environ_get_missing_with_default", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{})
		require.Equal(t, "fallback", mustRun(t, b, "import os; os.environ.get('MISSING', 'fallback')", fs.Handler()))
	})

	montyTest(t, "test_get_environ_len", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"A": "1", "B": "2", "C": "3"})
		require.Equal(t, int64(3), mustRun(t, b, "import os; len(os.environ)", fs.Handler()))
	})

	montyTest(t, "test_get_environ_contains", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"PRESENT": "value"})
		result := mustRun(t, b, "\nimport os\n('PRESENT' in os.environ, 'ABSENT' in os.environ)\n", fs.Handler())
		require.Equal(t, sandbox.Tuple{true, false}, result)
	})

	montyTest(t, "test_get_environ_keys", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_get_environ_values", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_get_environ_items", func(t *testing.T, b backend) {
		fs := newEnvFS(t, map[string]string{"X": "1", "Y": "2"})
		require.ElementsMatch(t, []any{sandbox.Tuple{"X", "1"}, sandbox.Tuple{"Y", "2"}}, mustRun(t, b, "import os; list(os.environ.items())", fs.Handler()))
	})

	montyTest(t, "test_get_environ_empty", func(t *testing.T, b backend) {
...
func requireStringDict(t testing.TB, want map[string]any, got any) {
	t.Helper()
	d, ok := got.(*sandbox.Dict)
	require.True(t, ok, "want *sandbox.Dict, got %T", got)
	m, ok := d.StringMap()
	require.True(t, ok)
...
func testFiles(t *testing.T) {
	...
	montyTest(t, "test_callback_file_read", func(t *testing.T, b backend) {
		var readCalls []sandbox.Path
		read := func(p sandbox.Path) (any, error) {
			readCalls = append(readCalls, p)
			return "content from " + string(p), nil
		}
		write := func(sandbox.Path, any) error { return nil }
	...
	t.Run("test_callback_file_write_direct", func(t *testing.T) {
		...
		var written []call
		read := func(sandbox.Path) (any, error) { return "", nil }
		write := func(p sandbox.Path, content any) error {
	...
	t.Run("test_callback_file_custom_permissions", func(t *testing.T) {
		file := osaccess.NewCallbackFile("/test/file.txt", func(sandbox.Path) (any, error) { return "", nil }, func(sandbox.Path, any) error { return nil }, 0o700)
		require.Equal(t, int64(0o700), file.Permissions())
	})

	t.Run("test_callback_file_repr", func(t *testing.T) {
		file := osaccess.NewCallbackFile("/test/file.txt", func(sandbox.Path) (any, error) { return "", nil }, func(sandbox.Path, any) error { return nil })
		require.Contains(t, file.String(), "CallbackFile(path=/test/file.txt")
	})

	montyTest(t, "test_custom_abstract_file", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_custom_abstract_file_mixed_with_memory_file", func(t *testing.T, b backend) {
		...
		require.Equal(t, sandbox.Tuple{"from custom", "from memory"}, mustRun(t, b, code, fs.Handler()))
	})
	...
	t.Run("test_os_access_direct_api", func(t *testing.T) {
		...
		contents := must[[]sandbox.Path](t)(fs.PathIterdir("/test"))
		sort.Slice(contents, func(i, j int) bool { return contents[i] < contents[j] })
		require.Equal(t, []sandbox.Path{"/test/file.txt", "/test/subdir"}, contents)
...
func testEdgeCases(t *testing.T) {
	montyTest(t, "test_root_directory", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/file.txt", "root file"))
		code := "\nfrom pathlib import Path\n(Path('/').is_dir(), sorted([str(p) for p in Path('/').iterdir()]))\n"
		require.Equal(t, sandbox.Tuple{true, []any{"/file.txt"}}, mustRun(t, b, code, fs.Handler()))
	})

	montyTest(t, "test_empty_file", func(t *testing.T, b backend) {
		fs := newFS(t, mem("/empty.txt", ""))
		code := "\nfrom pathlib import Path\n(Path('/empty.txt').read_text(), Path('/empty.txt').stat().st_size)\n"
		require.Equal(t, sandbox.Tuple{"", int64(0)}, mustRun(t, b, code, fs.Handler()))
	})

	montyTest(t, "test_large_nested_path", func(t *testing.T, b backend) {
	...
	montyTest(t, "test_special_characters_in_content", func(t *testing.T, b backend) {
		content := "line1\nline2\ttab\r\nwindows"
