...
func TestWasmMemoryLimit(t *testing.T) {
	t.Run("a session limit leaves normal wasm work alone", func(t *testing.T) {
		...
		s := wsmCheckout(t, p, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxMemory: 1024 * 1024}})
	...
	t.Run("an overrun the interpreter catches leaves the instance alive", func(t *testing.T) {
		...
		s := wsmCheckout(t, p, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxMemory: maxMemory}})
	...
	t.Run("exceeding the limit kills the instance and the pool recovers", func(t *testing.T) {
		ctx := testCtx(t)
		p := newPool(t, monty.BackendWasm, monty.Options{})
		s := wsmCheckout(t, p, monty.CheckoutOptions{Limits: &monty.ResourceLimits{MaxMemory: 1024}})
		...
		next := wsmCheckout(t, p, monty.CheckoutOptions{})
