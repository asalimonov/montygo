...
files=(
    proto/PROTO_REV montygo.go worker-wasm/Cargo.toml server/Cargo.toml server/src/version.rs
...
copy rev
sed -i.bak 's/^\(.*UpstreamRev *= *\)"[0-9a-f]*"/\1"00000000"/' "$tmp/rev/montygo.go" && rm "$tmp/rev/montygo.go.bak"
if out=$("$script" "$tmp/rev" 2>&1); then
    echo "FAIL drifted UpstreamRev passed" >&2; fail=1
elif [[ "$out" == *"montygo.go: expected UpstreamRev"* ]]; then
...
copy version
sed -i.bak 's/^\(.*MontyVersion *= *\)"[^"]*"/\1"9.9.9"/' "$tmp/version/montygo.go" && rm "$tmp/version/montygo.go.bak"
