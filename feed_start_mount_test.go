...
func TestFeedStartMounts(t *testing.T) {
	eachBackend(t, func(t *testing.T, b monty.Backend) {
		t.Run("mounts are re-supplied to loadSnapshot", func(t *testing.T) {
			...
			mount, err := monty.NewMountDir(monty.MountDirOptions{HostPath: dir, VirtualPath: "/data", Mode: "read-only"})
			require.NoError(t, err)
			t.Cleanup(func() { _ = mount.Close() })
			mounts := []*monty.MountDir{mount}
			code := "f()\nfrom pathlib import Path\nPath('/data/hello.txt').read_text()"

			first := newSession(t, b, monty.CheckoutOptions{})
			snap := fstFunction(t)(first.FeedStart(ctx, code, &monty.FeedOptions{Mount: mounts}))
...

			{
				...
				restored := fstFunction(t)(session.LoadSnapshot(ctx, blob, &monty.LoadSnapshotOptions{Mount: mounts}))
