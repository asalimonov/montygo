...
type clsCalculator struct {
	Value      int
	Memo       string `monty:"-"`
	lastKwargs montygo.Kwargs
}

func (c *clsCalculator) Add(n int) int { return c.Value + n }

func (c *clsCalculator) Scale(kw montygo.Kwargs) int {
	factor := int64(2)
...
func (c *clsCalculator) Combine(a, b int, kw montygo.Kwargs) string {
	c.lastKwargs = kw
	sep := ","
...
func (c *clsCalculator) Boom() error { return montygo.Raise("ValueError", "nope") }

func (c *clsCalculator) Fetch() *montygo.Future {
...
func (f *clsFlaky) GetAttr(name string) (any, error) {
	...
	case "value":
		return 1, nil
	case "boom":
		return nil, montygo.Raise("KeyError", "boom")
	case "raw":
...
type clsConfig struct{ options montygo.Kwargs }
...
func clsWrapDerivedWallet(_ string, v any) (any, error) {
	if w, ok := v.(*clsWallet); ok {
		return montygo.NewClassInstance(w, montygo.ClassInstanceOptions{EagerAttrs: montygo.All(), ConvertValue: clsWrapDerivedWallet})
	}
	return v, nil
}
...
func clsInstance(t testing.TB, obj any, opts montygo.ClassInstanceOptions) *montygo.ClassInstance {
	t.Helper()
	ci, err := montygo.NewClassInstance(obj, opts)
	require.NoError(t, err)
	return ci
}

func clsType[T any](t testing.TB, opts montygo.ClassTypeOptions) *montygo.ClassType {
	t.Helper()
	ct, err := montygo.NewClassType[T](opts)
	require.NoError(t, err)
	return ct
}

func clsRun(t testing.TB, b montygo.Backend, code string, inputs map[string]any) (any, error) {
	t.Helper()
	return run(t, b, code, runOptions{FeedOptions: montygo.FeedOptions{Inputs: inputs}})
}

func clsMustRun(t testing.TB, b montygo.Backend, code string, inputs map[string]any) any {
	t.Helper()
...
func clsConversionMessage(t testing.TB, err error) string {
	t.Helper()
	var ce *montygo.ConversionError
	require.ErrorAs(t, err, &ce)
	return ce.Message
}

func clsDict(pairs ...any) *montygo.Dict {
	d := montygo.NewDict()
...
func TestClassInstance(t *testing.T) {
	eachBackend(t, func(t *testing.T, b montygo.Backend) {
		t.Run("eagerAttrs all sends own non-underscore props", func(t *testing.T) {
			...
			x := clsInstance(t, g, montygo.ClassInstanceOptions{EagerAttrs: montygo.All()})
		...
		t.Run("eagerAttrs explicit list sends exactly those props", func(t *testing.T) {
			...
			x := clsInstance(t, c, montygo.ClassInstanceOptions{EagerAttrs: montygo.Names("value")})
		...
		t.Run("an attr outside the eager list raises AttributeError", func(t *testing.T) {
			...
			x := clsInstance(t, g, montygo.ClassInstanceOptions{EagerAttrs: montygo.Names()})
		...
		t.Run("the same instance round-trips across feeds in one session", func(t *testing.T) {
			...
			v, err := s.FeedRun(ctx, "x", &montygo.FeedOptions{Inputs: map[string]any{"x": clsInstance(t, g, montygo.ClassInstanceOptions{EagerAttrs: montygo.All()})}})
			require.NoError(t, err)
			require.Same(t, g, v)
			v, err = s.FeedRun(ctx, "y", &montygo.FeedOptions{Inputs: map[string]any{"y": clsInstance(t, g, montygo.ClassInstanceOptions{EagerAttrs: montygo.All()})}})
			require.NoError(t, err)
			require.Same(t, g, v)
		})

		t.Run("sync method call with args", func(t *testing.T) {
			c := clsInstance(t, &clsCalculator{Value: 5}, montygo.ClassInstanceOptions{AllowedMethods: montygo.Names("add")})
			require.Equal(t, int64(15), clsMustRun(t, b, "c.add(10)", map[string]any{"c": c}))
		})

		t.Run(`method call allowed by "all"`, func(t *testing.T) {
			c := clsInstance(t, &clsCalculator{Value: 5}, montygo.ClassInstanceOptions{AllowedMethods: montygo.All()})
			require.Equal(t, int64(15), clsMustRun(t, b, "c.add(10)", map[string]any{"c": c}))
		})
		...
		t.Run(`instance callMethod rejects __call__ even under allowedMethods "all"`, func(t *testing.T) {
			...
			x := clsInstance(t, tool, montygo.ClassInstanceOptions{AllowedMethods: montygo.All()})
		...
		t.Run("promise-returning method resolves via the future machinery", func(t *testing.T) {
			c := clsInstance(t, &clsCalculator{Value: 4}, montygo.ClassInstanceOptions{AllowedMethods: montygo.All()})
		...
		t.Run("denied method raises AttributeError", func(t *testing.T) {
			c := clsInstance(t, &clsCalculator{Value: 5}, montygo.ClassInstanceOptions{AllowedMethods: montygo.Names("scale")})
		...
		t.Run("no allowedMethods policy denies every method", func(t *testing.T) {
			c := clsInstance(t, &clsCalculator{Value: 5}, montygo.ClassInstanceOptions{})
			_, err := clsRun(t, b, "c.add(1)", map[string]any{"c": c})
			require.Equal(t, "AttributeError: 'clsCalculator' object has no attribute 'add'", clsRuntimeMessage(t, err))
		})
		...
		t.Run("lazy attr allowed by an explicit set", func(t *testing.T) {
			g := clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{LazyAttrs: montygo.Names("greeting")})
			require.Equal(t, "hello", clsMustRun(t, b, "g.greeting", map[string]any{"g": g}))
		})

		t.Run(`lazy attr allowed by "all"`, func(t *testing.T) {
			g := clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{LazyAttrs: montygo.All()})
			require.Equal(t, "hello", clsMustRun(t, b, "g.greeting", map[string]any{"g": g}))
		})

		t.Run("lazy attr outside the policy raises AttributeError", func(t *testing.T) {
			g := clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{LazyAttrs: montygo.Names("other")})
			_, err := clsRun(t, b, "g.greeting", map[string]any{"g": g})
			require.Equal(t, "AttributeError: 'clsGreeter' object has no attribute 'greeting'", clsRuntimeMessage(t, err))
		})

		t.Run("getattr/hasattr consult lazy attrs like g.attr", func(t *testing.T) {
			inputs := map[string]any{"g": clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{LazyAttrs: montygo.Names("greeting")})}
		...
		t.Run("underscore attrs never reach the host", func(t *testing.T) {
			...
			g := clsInstance(t, &clsGreeter{Greeting: "hello", Hidden: "secret"}, montygo.ClassInstanceOptions{
				LazyAttrs: montygo.All(),
		...
		t.Run("a throwing lazy getter raises its error in the sandbox", func(t *testing.T) {
			f := clsInstance(t, &clsFlaky{}, montygo.ClassInstanceOptions{LazyAttrs: montygo.All()})
		...
		t.Run("hasattr and getattr defaults do not swallow a lazy attribute host error", func(t *testing.T) {
			inputs := map[string]any{"f": clsInstance(t, &clsFlaky{}, montygo.ClassInstanceOptions{LazyAttrs: montygo.All()})}
		...
		t.Run("a convertValue throw on a lazy attr raises in the sandbox", func(t *testing.T) {
			g := clsInstance(t, &clsGreeter{Greeting: "hi"}, montygo.ClassInstanceOptions{
				LazyAttrs: montygo.All(),
				ConvertValue: func(name string, _ any) (any, error) {
					return nil, montygo.Raise("ValueError", "cannot convert "+name)
				},
			})
			code := "try:\n    g.greeting\n    r = 'unexpected'\nexcept ValueError as e:\n    r = str(e)\nr"
		...
		t.Run("NameLookupSnapshot.resumeAuto raises a lazy attribute host error in the sandbox", func(t *testing.T) {
			ctx := testCtx(t)
			s := newSession(t, b, montygo.CheckoutOptions{})
			snap, err := s.FeedStart(ctx, clsCatchBoom, &montygo.FeedOptions{Inputs: map[string]any{"f": clsInstance(t, &clsFlaky{}, montygo.ClassInstanceOptions{LazyAttrs: montygo.All()})}})
			require.NoError(t, err)
		...
		t.Run("convertValue transforms eager attrs and method returns", func(t *testing.T) {
			g := &clsGreeter{Greeting: "hello"}
			eager := clsInstance(t, g, montygo.ClassInstanceOptions{EagerAttrs: montygo.All(), ConvertValue: clsUpper})
			require.Equal(t, "HELLO", clsMustRun(t, b, "g.greeting", map[string]any{"g": eager}))
			methods := clsInstance(t, g, montygo.ClassInstanceOptions{AllowedMethods: montygo.All(), ConvertValue: clsUpper})
		...
		t.Run("a method returning a class instance is not auto-wrapped", func(t *testing.T) {
			w := clsInstance(t, &clsWallet{Balance: 100}, montygo.ClassInstanceOptions{EagerAttrs: montygo.All(), AllowedMethods: montygo.All()})
			code := "try:\n    w.pay(30)\n    r = 'unexpected'\nexcept TypeError as e:\n    r = str(e)\nr"
		...
		t.Run("a convertValue override chooses the derived instance policy", func(t *testing.T) {
			...
			opts := montygo.ClassInstanceOptions{EagerAttrs: montygo.All(), AllowedMethods: montygo.All(), ConvertValue: clsWrapDerivedWallet}
		...
		t.Run("a returned override-wrapped instance restores to the original object", func(t *testing.T) {
			w := clsInstance(t, &clsWallet{Balance: 100}, montygo.ClassInstanceOptions{AllowedMethods: montygo.All(), ConvertValue: clsWrapDerivedWallet})
		...
		t.Run("a sandbox-defined class instance surfaces as a MontyClassProxy", func(t *testing.T) {
			...
			require.True(t, montygo.Equal(clsDict("a", int64(1)), proxy.Attributes), montygo.Repr(proxy.Attributes))
		})
		...
		t.Run("a host-origin proxy from a restored session re-enters as a host-backed copy", func(t *testing.T) {
			ctx := testCtx(t)
			first := newSession(t, b, montygo.CheckoutOptions{})
			obj := clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{EagerAttrs: montygo.All()})
		...
		t.Run("a sandbox-defined dataclass instance reports isDataclass", func(t *testing.T) {
			...
			require.True(t, montygo.Equal(clsDict("x", int64(1), "y", int64(2)), proxy.Attributes), montygo.Repr(proxy.Attributes))
		})
		...
		t.Run("a duplicate wrapper id wrapping a different object is rejected", func(t *testing.T) {
			...
			a := clsInstance(t, &clsGreeter{Greeting: "hi"}, montygo.ClassInstanceOptions{ID: id})
			c := clsInstance(t, &clsGreeter{Greeting: "yo"}, montygo.ClassInstanceOptions{ID: id})
		...
		t.Run("a forged raw ClassInstance marker is rejected", func(t *testing.T) {
			forged := value.Instance{Type: value.Type{Name: "Point"}, ID: "x", Attrs: montygo.NewDict()}
		...
		t.Run("NameLookupSnapshot.resumeValue answers a lazy attribute lookup by hand", func(t *testing.T) {
			ctx := testCtx(t)
			s := newSession(t, b, montygo.CheckoutOptions{})
			g := clsInstance(t, &clsGreeter{Greeting: "hi"}, montygo.ClassInstanceOptions{LazyAttrs: montygo.All()})
		...
		t.Run("sandbox instantiation of a host class via ClassType", func(t *testing.T) {
			wrapper := clsType[clsCalculator](t, montygo.ClassTypeOptions{Init: true, InstanceAllowedMethods: montygo.All()})
		...
		t.Run("a constructed instance round-trips to a real host instance", func(t *testing.T) {
			wrapper := clsType[clsGreeter](t, montygo.ClassTypeOptions{Init: true})
		...
		t.Run("init false raises TypeError in the sandbox", func(t *testing.T) {
			wrapper := clsType[clsCalculator](t, montygo.ClassTypeOptions{})
		...
		t.Run("constructor kwargs arrive as a trailing options bag", func(t *testing.T) {
			wrapper := clsType[clsConfig](t, montygo.ClassTypeOptions{
				Init:                   true,
				InstanceAllowedMethods: montygo.All(),
				Constructor:            func(kw montygo.Kwargs) *clsConfig { return &clsConfig{options: kw} },
			})
		...
		t.Run("a denied method on a constructed instance raises AttributeError", func(t *testing.T) {
			wrapper := clsType[clsCalculator](t, montygo.ClassTypeOptions{Init: true, InstanceAllowedMethods: montygo.Names("add")})
		...
		t.Run("eagerAttrs on a ClassType sends static class constants", func(t *testing.T) {
			wrapper := clsType[clsShape](t, montygo.ClassTypeOptions{EagerAttrs: montygo.All(), Statics: clsShapeStatics()})
		...
		t.Run("a constructed instance keeps the ClassType id and name", func(t *testing.T) {
			wrapper := clsType[clsWallet](t, montygo.ClassTypeOptions{
		...
		t.Run("a constructed instance of another class gets a default ClassType", func(t *testing.T) {
			wrapper := clsType[clsFactory](t, montygo.ClassTypeOptions{
				Init:        true,
		...
		t.Run("an own constructor property does not change class identity", func(t *testing.T) {
			...
			wrapper := clsType[clsShadowing](t, montygo.ClassTypeOptions{
				Init:               true,
				InstanceEagerAttrs: montygo.All(),
			...
			require.Equal(t, "clsShadowing", clsInstance(t, instance, montygo.ClassInstanceOptions{}).Name())
			require.Same(t, wrapper, clsInstance(t, instance, montygo.ClassInstanceOptions{ClassType: wrapper}).ClassType())
			require.Same(t, wrapper, clsInstance(t, *instance, montygo.ClassInstanceOptions{ClassType: wrapper}).ClassType())
		...
		t.Run("an instance's type branch carries the ClassType's eager attrs", func(t *testing.T) {
			classType := clsType[clsShape](t, montygo.ClassTypeOptions{EagerAttrs: montygo.Names("SIDES"), Statics: clsShapeStatics()})
			inputs := map[string]any{"x": clsInstance(t, &clsShape{Size: 1}, montygo.ClassInstanceOptions{ClassType: classType})}
		...
		t.Run("type objects of two instances are one object", func(t *testing.T) {
			inputs := map[string]any{
				"a": clsInstance(t, &clsShape{Size: 1}, montygo.ClassInstanceOptions{}),
				"b": clsInstance(t, &clsShape{Size: 2}, montygo.ClassInstanceOptions{}),
			}
		...
		t.Run("a ClassType name override reaches the instance and its error message", func(t *testing.T) {
			classType := clsType[clsShape](t, montygo.ClassTypeOptions{Name: "Polygon"})
			inputs := map[string]any{"x": clsInstance(t, &clsShape{Size: 1}, montygo.ClassInstanceOptions{ClassType: classType})}
		...
		t.Run("name on a ClassInstance names its default ClassType, and clashes with classType", func(t *testing.T) {
			x := clsInstance(t, &clsShape{Size: 1}, montygo.ClassInstanceOptions{Name: "Poly"})
			require.Equal(t, "Poly", clsMustRun(t, b, "type(x).__name__", map[string]any{"x": x}))
			_, err := montygo.NewClassInstance(&clsShape{Size: 1}, montygo.ClassInstanceOptions{Name: "Poly", ClassType: clsType[clsShape](t, montygo.ClassTypeOptions{})})
		...
		t.Run("lazyAttrs on a ClassType serves class constants on demand", func(t *testing.T) {
			wrapper := clsType[clsShape](t, montygo.ClassTypeOptions{LazyAttrs: montygo.Names("SIDES"), Statics: clsShapeStatics()})
		...
		t.Run("allowedMethods on a ClassType exposes static methods", func(t *testing.T) {
			wrapper := clsType[clsShape](t, montygo.ClassTypeOptions{AllowedMethods: montygo.Names("double"), Statics: clsShapeStatics()})
		...
		t.Run("a denied static method uses the type-object wording", func(t *testing.T) {
			wrapper := clsType[clsShape](t, montygo.ClassTypeOptions{Statics: clsShapeStatics()})
		...
		t.Run("instantiate turns carry the class uuid as objectId", func(t *testing.T) {
			ctx := testCtx(t)
			s := newSession(t, b, montygo.CheckoutOptions{})
			wrapper := clsType[clsCalculator](t, montygo.ClassTypeOptions{Init: true})
		...
		t.Run(`JS object machinery is denied on an instance under "all", and the host object is untouched`, func(t *testing.T) {
			...
			inputs := map[string]any{"c": clsInstance(t, c, montygo.ClassInstanceOptions{AllowedMethods: montygo.All(), LazyAttrs: montygo.All()})}
			code := []string{
		...
		t.Run(`JS function machinery is denied on a class under "all"`, func(t *testing.T) {
			inputs := map[string]any{"Shape": clsType[clsShape](t, montygo.ClassTypeOptions{AllowedMethods: montygo.All(), LazyAttrs: montygo.All(), Statics: clsShapeStatics()})}
			code := []string{
		...
		t.Run("an explicit policy cannot name JS object machinery either", func(t *testing.T) {
			...
			wrapper := clsInstance(t, c, montygo.ClassInstanceOptions{
				AllowedMethods: montygo.Names(append([]string{"add"}, denied...)...),
				LazyAttrs:      montygo.Names(append([]string{"value"}, denied...)...),
			})
		...
		t.Run("a wrapped function cannot be invoked through Function.prototype", func(t *testing.T) {
			...
			inputs := map[string]any{"tool": clsInstance(t, tool, montygo.ClassInstanceOptions{AllowedMethods: montygo.All(), LazyAttrs: montygo.All()})}
			code := []string{
		...
		t.Run(`"all" exposes methods the class defines, not callables stored on the instance`, func(t *testing.T) {
			...
			all := map[string]any{
				"b": clsInstance(t, bag, montygo.ClassInstanceOptions{AllowedMethods: montygo.All()}),
				"Bag": clsType[clsBag](t, montygo.ClassTypeOptions{AllowedMethods: montygo.All(), Statics: map[string]any{
					"make":  func() string { return "made" },
					"Inner": clsType[clsBagInner](t, montygo.ClassTypeOptions{}),
				}}),
			}
			...
			explicit := clsInstance(t, bag, montygo.ClassInstanceOptions{AllowedMethods: montygo.Names("run")})
		...
		t.Run("a __proto__ keyword argument never reaches the host", func(t *testing.T) {
			c := &clsCalculator{Value: 5}
			inputs := map[string]any{"c": clsInstance(t, c, montygo.ClassInstanceOptions{AllowedMethods: montygo.All()})}
		...
		t.Run("an explicit id is validated and lowercased, and still round-trips to the original", func(t *testing.T) {
			...
			wrapper := clsInstance(t, g, montygo.ClassInstanceOptions{ID: "ABCDEF01-2345-4678-89AB-CDEF01234567"})
			...
			classType := clsType[clsShape](t, montygo.ClassTypeOptions{ID: "12345678-1234-4123-8123-123456789ABC"})
			...
			var ve *montygo.ValueError
			_, err := montygo.NewClassInstance(g, montygo.ClassInstanceOptions{ID: "not-a-uuid"})
			require.ErrorAs(t, err, &ve)
			require.Equal(t, `ClassInstance id must be a canonical uuid string, got "not-a-uuid"`, ve.Message)
			_, err = montygo.NewClassType[clsShape](montygo.ClassTypeOptions{ID: "12345678123441238123123456789abc"})
			require.ErrorAs(t, err, &ve)
		...
		t.Run("a returned host class resolves to the class object when registered", func(t *testing.T) {
			...
			classType := clsType[clsShape](t, montygo.ClassTypeOptions{})
			require.Same(t, classType, clsMustRun(t, b, "Shape", map[string]any{"Shape": classType}))

			x := clsInstance(t, &clsShape{Size: 1}, montygo.ClassInstanceOptions{})
			v := clsMustRun(t, b, "type(x)", map[string]any{"x": x})
			ct, ok := v.(*montygo.ClassType)
			require.True(t, ok, "%T", v)
			...
			first, ok := items[0].(*montygo.ClassType)
		...
		t.Run("a returned host class stays a Type marker when the session never registered it", func(t *testing.T) {
			ctx := testCtx(t)
			first := newSession(t, b, montygo.CheckoutOptions{})
			obj := clsInstance(t, &clsGreeter{Greeting: "hello"}, montygo.ClassInstanceOptions{})
			_, err := first.FeedRun(ctx, "x = obj", &montygo.FeedOptions{Inputs: map[string]any{"obj": obj}})
			require.NoError(t, err)
			blob, err := first.Dump(ctx)
			require.NoError(t, err)
		...
		t.Run("a forged host-class Type marker is rejected, builtin type markers still pass prepare", func(t *testing.T) {
			forged := montygo.Type{Name: "Shape", ID: "12345678-1234-4123-8123-123456789abc", Origin: montygo.OriginHost}
			...
			require.Equal(t, true, clsMustRun(t, b, "x is int", map[string]any{"x": montygo.Type{Name: "int", Origin: montygo.OriginBuiltin}}))
		})
	})
}
