...
os.listdir()
os.rename('./sub/../src', '../dst')`, &montygo.FeedOptions{
			Cwd: "/data",
		...
		require.Equal(t, []any{
			[]any{"Path.exists", []any{montygo.Path("/data/file.txt")}},
			[]any{"Path.exists", []any{montygo.Path("/other/file.txt")}},
			[]any{"Path.iterdir", []any{montygo.Path("/data")}},
			[]any{"Path.rename", []any{montygo.Path("/data/src"), montygo.Path("/dst")}},
		}, calls)
	})
	...
	t.Run("a time decodes over the wasm transport", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, montygo.Time{}, v)

		v, err = s.FeedRun(ctx, "import datetime\ndatetime.time(23, 59, 59, 999999, datetime.timezone(datetime.timedelta(hours=-5)))", nil)
		require.NoError(t, err)
		require.Equal(t, montygo.Time{Hour: 23, Minute: 59, Second: 59, Microsecond: 999999, OffsetSeconds: wsmInt32(-18000)}, v)
	})
	...
	t.Run("a time round-trips through the wasm transport", func(t *testing.T) {
		...
		aware := montygo.Time{Hour: 1, Minute: 2, Second: 3, Microsecond: 4, OffsetSeconds: wsmInt32(7200), TimezoneName: wsmString("P2"), Fold: 1}
		...
		utc := montygo.Time{Hour: 12, OffsetSeconds: wsmInt32(0)}
