...
func limLimits(l montygo.ResourceLimits) montygo.CheckoutOptions {
	return montygo.CheckoutOptions{Limits: &l}
}
...
func TestLimits(t *testing.T) {
	eachBackend(t, func(t *testing.T, b montygo.Backend) {
		...
		t.Run("run with limits", func(t *testing.T) {
			limits := limLimits(montygo.ResourceLimits{MaxDuration: 5 * time.Second})
			require.Equal(t, int64(2), mustRun(t, b, "1 + 1", runOptions{CheckoutOptions: limits}))
		})

		t.Run("recursion limit", func(t *testing.T) {
			code := "\ndef recurse(n):\n    if n <= 0:\n        return 0\n    return 1 + recurse(n - 1)\n\nrecurse(10)\n"
			_, err := run(t, b, code, runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxRecursionDepth: 5})})
			rerr := limRuntimeError(t, err)
		...
		t.Run("recursion limit ok", func(t *testing.T) {
			code := "\ndef recurse(n):\n    if n <= 0:\n        return 0\n    return 1 + recurse(n - 1)\n\nrecurse(5)\n"
			v := mustRun(t, b, code, runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxRecursionDepth: 100})})
		...
		t.Run("memory limit", func(t *testing.T) {
			...
			_, err := run(t, b, code, runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxMemory: maxMemory})})
		...
		t.Run("memory limit accepts values above u32 max", func(t *testing.T) {
			limits := limLimits(montygo.ResourceLimits{MaxMemory: 1 << 33})
			require.Equal(t, int64(2), mustRun(t, b, "1 + 1", runOptions{CheckoutOptions: limits}))
		})

		t.Run("limits with inputs", func(t *testing.T) {
			v := mustRun(t, b, "x * 2", runOptions{
				CheckoutOptions: limLimits(montygo.ResourceLimits{MaxDuration: 5 * time.Second}),
				FeedOptions:     montygo.FeedOptions{Inputs: map[string]any{"x": 21}},
			})
			require.Equal(t, int64(42), v)
		})

		t.Run("pow memory limit", func(t *testing.T) {
			_, err := run(t, b, "2 ** 10000000", runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxMemory: 1_000_000})})
		...
		t.Run("lshift memory limit", func(t *testing.T) {
			_, err := run(t, b, "1 << 10000000", runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxMemory: 1_000_000})})
		...
		t.Run("mult memory limit", func(t *testing.T) {
			code := "\nbig = 2 ** 4000000\nresult = big * big\n"
			_, err := run(t, b, code, runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxMemory: 1_000_000})})
		...
		t.Run("small operations within limit", func(t *testing.T) {
			v := mustRun(t, b, "2 ** 1000", runOptions{CheckoutOptions: limLimits(montygo.ResourceLimits{MaxMemory: 1_000_000})})
			got, ok := v.(*big.Int)
		...
		t.Run("suspension limit", func(t *testing.T) {
			...
			_, err := run(t, b, code, runOptions{
				CheckoutOptions: limLimits(montygo.ResourceLimits{MaxSuspensions: 3}),
				FeedOptions:     montygo.FeedOptions{ExternalLookup: map[string]any{"fetch": fetch}},
			})
			rerr := limRuntimeError(t, err)
		...
		t.Run("suspension limit leaves the session usable", func(t *testing.T) {
			ctx := testCtx(t)
			session := newSession(t, b, limLimits(montygo.ResourceLimits{MaxSuspensions: 1}))
			feed := &montygo.FeedOptions{ExternalLookup: map[string]any{"fetch": func(string) string { return "ok" }}}
		...
		t.Run("restored session keeps its suspension limit with a fresh count", func(t *testing.T) {
			ctx := testCtx(t)
			feed := &montygo.FeedOptions{ExternalLookup: map[string]any{"fetch": func(string) string { return "ok" }}}
			first := newSession(t, b, limLimits(montygo.ResourceLimits{MaxSuspensions: 1}))
			...
			restored := newSession(t, b, montygo.CheckoutOptions{})
