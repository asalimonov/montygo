...
- `supervisor/docker` derives its image tag from `BindingVersion()`: the exact version, then the release it was built from. `0.3.0` yields `0.3.0`; `0.3.0-3f2a9c1-dirty` yields that tag, then `0.3.0`; a Go pseudo-version yields its base release; `(devel)` and `0.0.0-unknown` yield nothing and are an `OptionError`. Only releases are published, so `make docker-build` also tags `$(GHCR_IMAGE):$(VERSION)` and a development build resolves from the local daemon. `docker.Options.Image`, `docker.Options.Version`, `MONTYGO_DOCKER_IMAGE` and `MONTYGO_DOCKER_VERSION` override it. See `supervisor.md`.
