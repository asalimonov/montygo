...
- The image is tagged `$(IMAGE):$(VERSION)`, `$(IMAGE):latest` and `$(GHCR_IMAGE):$(VERSION)`, and labelled `org.opencontainers.image.version=$(VERSION)`, `org.opencontainers.image.revision=<montygo commit>` and `io.montygo.monty-rev=<MONTY_REV>`.
- `montygo.NewDocker` derives its image tag from `BindingVersion()`: the exact version, then the release it was built from. `0.3.0` yields `0.3.0`; `0.3.0-3f2a9c1-dirty` yields that tag, then `0.3.0`; a Go pseudo-version yields its base release; `(devel)` and `0.0.0-unknown` yield nothing and are an `OptionError`. Only releases are published, so `make docker-build` also tags `$(GHCR_IMAGE):$(VERSION)` and a development build resolves from the local daemon. `DockerOptions.Image`, `DockerOptions.Version`, `MONTYGO_DOCKER_IMAGE` and `MONTYGO_DOCKER_VERSION` override it. See `supervisor.md`.
