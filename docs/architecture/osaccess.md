...
- `Ops` has one method per OS call name. `Dispatch` routes `(name, args, kwargs)` to it and maps `ErrNotImplemented` to `montygo.NotHandled`, so an embedding `Base` declines everything it does not override.
...
- Errors are `montygo.Raise` values with CPython's exception types and errno messages, so they cross into the sandbox typed.
