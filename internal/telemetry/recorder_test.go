...
func TestSpanEndSkippedAfterUninstall(t *testing.T) {
	...
	sp := Current().StartSpan(context.Background(), "session {script_name}", nil, o)
...
func TestSyntheticSpanJoinsParentTrace(t *testing.T) {
	...
	sp := Current().StartSpan(trace.ContextWithSpanContext(context.Background(), parent), "run code", nil, nil)
